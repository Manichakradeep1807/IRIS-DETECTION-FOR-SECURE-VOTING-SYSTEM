...
def print_status(message, status="INFO"):
    ...
    print("[{}] {icon} {message}".format(timestamp))
...
def test_python_environment():
    ...
    try:
        ...
        print_status("Python Version: {}".format(python_version.split()[0]), "SUCCESS")
        ...
        print_status("Python environment test failed: {}".format(e), "ERROR")
        return False
...
def test_core_dependencies():
    ...
    for module, description in dependencies.items():
        try:
            ...
            print_status("{}: Available".format(description), "SUCCESS")
            results[module] = True
            
        except ImportError:
            print_status("{}: MISSING".format(description), "ERROR")
            results[module] = False
            missing_deps.append(module)
    
    if missing_deps:
        print_status("Missing dependencies: {}".format(missing_deps), "WARNING")
...
def test_project_files():
    ...
    for file in required_files:
        if os.path.exists(file):
            print_status("File {}: Found".format(file), "SUCCESS")
        else:
            print_status("File {}: MISSING".format(file), "ERROR")
            missing_files.append(file)
    
    # Check optional files
    for file in optional_files:
        if os.path.exists(file):
            print_status("Optional file {}: Found".format(file), "SUCCESS")
        else:
            print_status("Optional file {}: Not found".format(file), "WARNING")
    
    # Check directories
    for directory in required_dirs:
        if os.path.exists(directory):
            print_status("Directory {}: Found".format(directory), "SUCCESS")
        else:
            print_status("Directory {}: MISSING".format(directory), "ERROR")
...
def test_voice_commands():
    ...
    try:
        ...
        if hasattr(voice_system, 'command_patterns'):
            ...
            print_status("Found {} command patterns".format(len(patterns)), "SUCCESS")
            
            # Test key commands
            key_commands = ['start_recognition', 'take_photo', 'show_gallery', 'train_model']
            for cmd in key_commands:
                if cmd in patterns:
                    print_status("Command '{}': Available ({len(patterns[cmd])} patterns)".format(cmd), "SUCCESS")
                else:
                    print_status("Command '{}': Missing".format(cmd), "ERROR")
        ...
        print_status("Voice commands test failed: {}".format(e), "ERROR")
        return False
...
def test_model_files():
    ...
    for file in model_files:
        if os.path.exists(file):
            ...
            print_status("Model file {}: Found ({size:.1f} MB)".format(file), "SUCCESS")
        else:
            print_status("Model file {}: Missing".format(file), "WARNING")
    
    # Check optional model files
    for file in optional_model_files:
        if os.path.exists(file):
            size = os.path.getsize(file) / (1024 * 1024)  # MB
            print_status("Optional model {}: Found ({size:.1f} MB)".format(file), "SUCCESS")
        else:
            print_status("Optional model {}: Not found".format(file), "INFO")
    ...
    if os.path.exists('sample_dataset'):
        ...
        print_status("Sample dataset: {} person directories".format(len(person_dirs)), "SUCCESS")
    else:
    ...
    try:
        ...
        if cap.isOpened():
            ...
            if ret:
                ...
                print_status("Camera: Available ({}x{height})".format(width), "SUCCESS")
                cap.release()
                return True
            else:
        ...
        print_status("Camera test failed: {}".format(e), "ERROR")
        return False
...
def test_gui_components():
    ...
    try:
        ...
        print_status("GUI components test failed: {}".format(e), "ERROR")
        return False
...
def test_main_application():
    ...
    try:
        ...
        print_status("Main application test failed: {}".format(e), "ERROR")
        return False
...
if __name__ == "__main__":
    ...
    print_status("Tests completed: {}".format(total_tests), "INFO")
    print_status("Tests passed: {}".format(passed_tests), "SUCCESS" if passed_tests == total_tests else "WARNING")
    print_status("Tests failed: {}".format(total_tests - passed_tests), "ERROR" if passed_tests < total_tests else "SUCCESS")
    
    # Detailed results
    for test_name, result in results.items():
        status = "PASSED" if result else "FAILED"
        icon = "✅" if result else "❌"
        print_status("{}: {status}".format(test_name.upper()), "SUCCESS" if result else "ERROR")
    ...
    print("\nDiagnosis completed at: {}".format(datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
    print("=" * 70)
