...
def create_demo_gallery():
    ...
    for i in range(8):
        ...
        # Add labels
        cv2.putText(composite, "Demo Person {} - Confidence: 0.{85+i}".format(i+1), 
...
def create_gallery_window(captured_images, title="Live Iris Gallery Demo"):
    ...
    # Add subtitle
    subtitle_text = "{} Images Captured - Live Updates".format(total_images)
    cv2.putText(gallery, subtitle_text, (padding, 45), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)
    
    # Add timestamp
    timestamp_text = "Updated: {}".format(datetime.now().strftime('%H:%M:%S'))
    ...
    for i, capture_data in enumerate(captured_images):
        ...
        # Add image info
        info_text = "#{} Person {capture_data['person_id']}".format(i+1)
        cv2.putText(gallery, info_text, (x, y-8), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.35, (255, 255, 0), 1)
        
        # Add confidence
        conf_text = "Conf: {}".format(capture_data['confidence']:.2f)
...
def demo_live_gallery():
    ...
    try:
        ...
        for i in range(len(demo_images) + 1):
            ...
            gallery = create_gallery_window(current_images, "Live Gallery Demo - Step {}".format(i))
            
            if gallery is not None:
                cv2.imshow('Live Iris Gallery Demo', gallery)
                print("📸 Gallery updated: {} image(s) captured".format(i))
        ...
        if demo_images:
            ...
            print("   📊 {} images in gallery".format(len(demo_images)))
