...
def check_system_components():
    ...
    for name, filename in components.items():
        if os.path.exists(filename):
            ...
            print("   ✅ {} {filename} ({size:,} bytes)".format(name:20))
        else:
            print("   ❌ {} {filename} (missing)".format(name:20))
...
def check_model_files():
    ...
    for name, filepath in model_files.items():
        if os.path.exists(filepath):
            ...
            total_size += size
            print("   ✅ {} {filepath} ({size:,} bytes)".format(name:15))
        else:
            print("   ❌ {} {filepath} (missing)".format(name:15))
    
    print("   📊 Total model size: {} bytes ({total_size/1024/1024:.1f} MB)".format(total_size:,))
    return total_size > 0
    ...
    for name, folder in datasets.items():
        if os.path.exists(folder):
            ...
            if os.path.isdir(folder):
                ...
                if subdirs:
                    print("   ✅ {} {folder}/ ({len(subdirs)} persons, {len(files)} images)".format(name:15))
                else:
                    print("   ✅ {} {folder}/ ({len(files)} images)".format(name:15))
            else:
                print("   ✅ {} {folder}/ ({len(files)} images)".format(name:15))
        else:
            print("   ❌ {} {folder}/ (missing)".format(name:15))
...
def check_database():
    ...
    for name, db_file in databases.items():
        if os.path.exists(db_file):
            ...
            print("   ✅ {} {db_file} ({size:,} bytes)".format(name:15))
            ...
            if db_file == "iris_system.db":
                try:
                    ...
                    print("      📊 {} persons enrolled".format(person_count))
                    print("      📋 {} access log entries".format(log_count))
                    
                    conn.close()
                except Exception as e:
                    print("      ⚠️ Error reading database: {}".format(e))
        else:
            print("   ❌ {} {db_file} (missing)".format(name:15))
...
def check_new_features():
    ...
    if os.path.exists("captured_iris"):
        ...
        print("   ✅ Capture Folder    captured_iris/ ({} images)".format(len(files)))
    ...
    if os.path.exists("live_recognition.py"):
        with open("live_recognition.py", "r") as f:
            ...
            for feature_name, exists in features:
                ...
                print("   {} {feature_name:15} {'Implemented' if exists else 'Missing'}".format(status))
...
def main():
    ...
    print("=" * 60)
    print("📅 Status check: {}".format(datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
    print()
