...
def test_imports():
    ...
    try:
        ...
        issues.append(f"opencv-python: {e}")
        
    try:
        import matplotlib.pyplot as plt
        print("   ✅ matplotlib")
    except ImportError as e:
        issues.append(f"matplotlib: {e}")
    ...
    try:
        ...
        issues.append(f"scikit-learn: {e}")
        
    try:
        import pyttsx3
        print("   ✅ pyttsx3")
    except ImportError as e:
        issues.append(f"pyttsx3: {e}")
    ...
    if os.path.exists('model/X.txt.npy') and os.path.exists('model/Y.txt.npy'):
        try:
            ...
            if len(X) == 0:
                ...
                issues.append(f"Incorrect image shape: {X.shape[1:]}, expected (64, 64, 3)")
            
        except Exception as e:
            issues.append(f"Error loading training data: {e}")
    else:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                issues.append(f"Failed to create sample dataset: {result.stderr}")
        except Exception as e:
            issues.append(f"Error creating sample dataset: {e}")
...
def test_model_functions():
    ...
    try:
        ...
        issues.append(f"Model function error: {e}")
...
def test_analytics_functions():
    ...
    try:
        ...
        issues.append(f"Analytics function error: {e}")
...
def test_image_processing():
    ...
    try:
        ...
        if img is not None:
            print(f"   ✅ Image loading successful: {img.shape}")
        else:
        ...
        if resized.shape == (64, 64, 3):
            ...
            issues.append(f"Image resizing failed: {resized.shape}")
        ...
        if 0 <= normalized.min() and normalized.max() <= 1:
            ...
            issues.append(f"Image normalization failed: range [{normalized.min()}, {normalized.max()}]")
            
    except Exception as e:
        issues.append(f"Image processing error: {e}")
...
def test_main_gui_functions():
    ...
    try:
        ...
        issues.append(f"Main GUI function error: {e}")
        print(f"   ❌ Main GUI function error: {e}")
...
def run_specific_function_tests():
    ...
    try:
        ...
        issues.append(f"Analytics test error: {e}")
    ...
    try:
        ...
        issues.append(f"Model loading test error: {e}")
