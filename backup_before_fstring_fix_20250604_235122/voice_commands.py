...
class VoiceCommandSystem:
    ...
    def _initialize_speech_recognition(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Microphone initialization failed: {mic_error}")
    ...
    def _listen_worker(self):
        ...
        while self.is_listening:
            try:
                ...
                logger.error(f"Error in voice recognition worker: {e}")
                time.sleep(1)
    ...
    def _process_command(self, command_text):
        ...
        if matched_command:
            logger.info(f"Executing voice command: {matched_command}")
            self._execute_command(matched_command, command_text)
        else:
            # Unknown command
            unknown_msg = self._get_localized_text("voice_unknown", f"Unknown command: {command_text}")
    ...
    def _handle_voice_status(self):
        ...
        self.speak(f"Voice system status: {status}")
