...
class PerformanceMonitor:
        ...
        def decorator(func):
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    self.log_metric(f"{metric_name}_execution_time", execution_time)
                    self.log_metric(f"{metric_name}_success_rate", 1 if success else 0)
    ...
    def plot_performance_trends(self, metric_name, hours=24, save_path=None):
        ...
        if not recent_data:
            print(f"No recent data for metric: {metric_name}")
            return
    ...
    def get_active_alerts(self):
        ...
        if health['recognition_success_rate'] < 80:
            alerts.append({
                'level': 'WARNING',
                'message': f"Low success rate: {health['recognition_success_rate']:.1f}%"
            })
