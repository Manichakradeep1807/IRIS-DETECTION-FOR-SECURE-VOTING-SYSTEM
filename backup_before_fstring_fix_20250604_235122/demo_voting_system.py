...
def demo_voting_system():
    ...
    def demo_create_sample_data():
        ...
        try:
            ...
            messagebox.showinfo("Sample Data Created", 
                               f"✅ Created {votes_created} sample votes!\n\n"
    ...
    def show_current_stats():
        ...
        try:
            ...
            stats_text += f"Total Votes: {results['total_votes']}\n"
            stats_text += f"Total Voters: {results['total_voters']}\n"
            stats_text += f"Turnout: {(results['total_voters'] / 108) * 100:.1f}%\n\n"
            
            if results['total_votes'] > 0:
                stats_text += "🏆 TOP PARTIES:\n"
                for result in sorted(results['results'], key=lambda x: x['votes'], reverse=True)[:3]:
                    if result['votes'] > 0:
                        stats_text += f"{result['symbol']} {result['party']}: {result['votes']} votes\n"
            else:
