...
class PermanentFStringFixer:
    ...
    def convert_fstring_to_format(self, content):
        ...
        # Pattern 1: f"text {variable}" -> "text {}".format(variable)
        pattern1 = r'f"([^"]*?)\{([^}]+?)\}([^"]*?)"'
        def replace1(match):
        ...
        # Pattern 2: f'text {variable}' -> 'text {}'.format(variable)
        pattern2 = r"f'([^']*?)\{([^}]+?)\}([^']*?)'"
        def replace2(match):
            before, var, after = match.groups()
        ...
        # Pattern 3: Multiple variables f"text {var1} more {var2}"
        # This is more complex, let's handle common cases
        
        # Pattern for f"text {var1} more {var2}" with 2 variables
        pattern3 = r'f"([^"]*?)\{([^}]+?)\}([^"]*?)\{([^}]+?)\}([^"]*?)"'
        def replace3(match):
        ...
        def replace5(match):
            ...
            return f'"{before}{{{}}}{mid1}{{{}}}{mid2}{{{}}}{after}".format({var1}, {var2}, {var3})'
        ...
        def replace6(match):
            ...
            return f'"{before}{{{}}}{mid1}{{{}}}{mid2}{{{}}}{mid3}{{{}}}{after}".format({var1}, {var2}, {var3}, {var4})'
    ...
    def process_file(self, filepath):
        ...
        try:
            ...
            if conversions > 0:
                ...
                print(f"   ✅ {filepath}: {conversions} f-strings converted")
                return conversions
            else:
                return 0
    ...
    def verify_fix(self):
        ...
        if remaining_fstrings:
            print(f"   ⚠️ Found {len(remaining_fstrings)} files with remaining f-strings:")
            for filepath, count in remaining_fstrings:
                print(f"      • {filepath}: {count} f-strings")
            return False
        else:
    ...
    def run_permanent_fix(self):
        ...
        print(f"🔧 F-strings converted: {fstrings_converted}")
        ...
        if overall_success:
            ...
            print(f"   • Converted {fstrings_converted} f-strings across {files_processed} files")
