...
try:
    ...
    print("⚠️ Voting system support not available: {}".format(str(e)))
...
def predictChange():
    ...
    try:
        ...
        if image is not None:
            ...
            if VOTING_SYSTEM_SUPPORT and confidence >= 70:  # Only allow voting with good confidence
                ...
                if voting_system.has_voted(predict):
                    ...
                    text.insert(tk.END, "⚠️ Person {} has already voted!\n".format(predict))
                    text.insert(tk.END, "   Vote cast for: {} {}\n".format(existing_vote['party'], existing_vote['symbol']))
                    text.insert(tk.END, "   Time: {}\n\n".format(existing_vote['timestamp']))

                    # Show existing vote info
                    messagebox.showinfo(
                        "Already Voted",
            ...
            try:
                ...
                if original_img is not None:
                    ...
                    result_text = f'Person {predict} - Confidence: {confidence:.1f}%'
                    ...
                    result_filename = f'recognition_result_{predict}_{confidence:.1f}percent.jpg'
                    cv2.imwrite(result_filename, original_img)
                    text.insert(tk.END, f"📸 Result image saved: {result_filename}\n")
                ...
                if os.path.exists('test.png'):
                    ...
                    if extracted_img is not None:
                        ...
                        features_filename = f'extracted_features_{predict}.jpg'
                        cv2.imwrite(features_filename, extracted_img)
                        text.insert(tk.END, f"📸 Features image saved: {features_filename}\n")
                ...
                text.insert(tk.END, f"⚠️ Visualization error (non-critical): {str(viz_error)}\n")
        ...
        text.insert(tk.END, f"❌ Error during recognition: {str(e)}\n\n")
...
def voice_database_status():
    ...
    try:
        ...
        db_files = ['iris_recognition.db', 'system_data.db']
...
def voice_version_info():
    ...
    try:
        ...
        text.insert(tk.END, "   Features: Voice Commands, Live Recognition\n")
...
def show_voting_menu():
    ...
    try:
        ...
        text.insert(tk.END, "❌ Voting System Error: {}\n".format(str(e)))
        messagebox.showerror("Voting System Error", "Could not open voting system: {}".format(str(e)))
    ...
    try:
        ...
        def authenticate_and_vote():
            ...
            try:
                ...
                if image is not None:
                    try:
                        ...
                        try:
                            ...
                            text.insert(tk.END, "✅ Authentication accepted! Confidence: {:.1f}%\n".format(confidence))
                            ...
                            if VOTING_SYSTEM_SUPPORT:
                                try:
                                    ...
                                    text.insert(tk.END, "❌ Voting system error: {}\n\n".format(str(voting_error)))
                                    main.update()
                                    messagebox.showerror("Voting System Error",
                                                       "Error accessing voting system:\n{}".format(str(voting_error)))
                            else:
                                messagebox.showerror("Voting System Error", "Voting system is not available.")
                        except Exception as prediction_error:
                            text.insert(tk.END, "❌ Model prediction error: {}\n\n".format(str(prediction_error)))
                            main.update()
                            messagebox.showerror("Prediction Error",
                                               "Error during model prediction:\n{}".format(str(prediction_error)))

                    except Exception as processing_error:
                        text.insert(tk.END, "❌ Image processing error: {}\n\n".format(str(processing_error)))
                        main.update()
                ...
                text.insert(tk.END, "❌ Unexpected authentication error: {}\n\n".format(str(e)))
                main.update()
                import traceback
                traceback.print_exc()
                messagebox.showerror("Authentication Error",
                                   "Unexpected error during authentication:\n\n{}\n\n"
                                   "Please check the console for detailed error information.".format(str(e)))
        ...
        text.insert(tk.END, "❌ Enhanced voting error: {}\n\n".format(str(e)))
        messagebox.showerror("Enhanced Voting Error", "Could not start enhanced voting: {}".format(str(e)))
