...
class HeadlessIrisCapture:
    ...
    def capture_iris_image(self, iris_image, eye_roi, person_id=1, confidence=0.85):
        ...
        try:
            ...
            print(f"📸 Iris captured #{self.captured_count}: Person {person_id} (Confidence: {confidence:.2f})")
    ...
    def run_capture_demo(self, duration=30):
        ...
        try:
            while time.time() - start_time < duration:
                ...
                if int(elapsed) % 5 == 0 and elapsed > 0:
                    ...
                    print(f"⏱️  Progress: {elapsed:.0f}s elapsed, {remaining:.0f}s remaining, {self.captured_count} captures")
        ...
        print(f"   Iris images captured: {self.captured_count}")
        print(f"   Images saved in: {self.capture_folder}/")
        
        # List captured files
        if os.path.exists(self.capture_folder):
            files = [f for f in os.listdir(self.capture_folder) if f.endswith('.jpg')]
            if files:
                print(f"\n📁 Captured files:")
